...
# 25    Decoding: Problematic values
# 26    Decoding:
# 27    Decoding:
# 28    Decoding:
# 29    Decoding:
...
def get_bad_spills(server, schema):
    ...
    bad_spill_set = set()
    spill_set = set()
    # reset_spill_dataquality_bit(schema)
...
def clear_bad_event_dimuons(server, schema, table, merged_schema):
    ...
    try:
        ...
        cur.execute(query % (schema, table, schema, table, merged_schema))
...
def reset_spill_dataquality_bit(schema):
